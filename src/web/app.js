  ...
  const observer = new MutationObserver(() => {
    ...
    if (vid && !vid._trimListener) {
      ...
      const onMeta = () => {
        ...
        syncFromInputs();
        // Range-loop: clamp playback within the selected slice
        // loop=true is the safety net (video never stops); timeupdate enforces the range
        vid.addEventListener("timeupdate", () => {
          if (videoDuration <= 0) return;
          const startSec = startFrac * videoDuration;
          const endSec   = endFrac   * videoDuration;
          if (vid.currentTime >= endSec - 0.08) {
            vid.currentTime = startSec;          // end handle reached — loop back
          } else if (startFrac > 0 && vid.currentTime < startSec) {
            vid.currentTime = startSec;          // native loop jumped to 0 past start handle
          }
        });
      };
